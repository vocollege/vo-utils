...
// import VoSelectField from "components/VoSelectField";
...
import { VoLoader } from "../index";
import Editor from "components/Editor";
import { fakeMutation } from "api";
import { useStylesLayout } from "ui/styles/layout";
...
const Form = (props: FormProps) => {
  ...
  const isCreateNew = () => {
    return params[primaryField || "id"] === "create";
  };
  ...
  const handleChangeAuthor = (field: string, item: EntityPickerItem) => {
    let value = `${item.id} - ${item.title}`;
    dispatch({ field: field, value: value });
    setValue(field, value, { shouldValidate: true, shouldDirty: true });
  };

  const handleAuthorFieldReset = (field: string, type: string) => {
    handleChangeAuthor(field, {
      id: VoAuth.currentUser.id,
      title: VoAuth.currentUser.name,
      type: type,
    });
  };
      ...
      case "author_field":
        return (
          <EntityField
            ...
            label={field.label}
            value={state[field.name]}
            onChange={(item) => handleChangeAuthor(field.name, item)}
            onReset={() =>
              handleAuthorFieldReset(field.name, field.params?.types)
            }
            required={field?.required}
            types={field.params?.types}
  ...
  const handleCompleted = (data: any) => {
    ...
    history.push(paths.back, { refetchPages: true });
  };
      ...
      {!queryLoading && !queryError && (
        <form className={classes.root} noValidate autoComplete="off">
          ...
          <FormToolbar
            ...
            className="vo-global__content-toolbar"
          />
